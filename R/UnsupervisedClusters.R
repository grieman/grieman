...
UnsupervisedClusters <- function(datain,  plottype = "fan", forceclusters = NULL, classification = NULL){
  ...
  colors = c("#004990", "#a7a9ac","#00AFD5","#cddc38","#bed3e4","#000000","#c6dbd8","#246987","#768d99","blue")

  #### CLUSTER ANALYSIS ####
  #finds the best number of clusters. Centroid method is used to better handle outliers
  if (is.null(forceclusters)){
    clustnums <- max.col(t(table(NbClust::NbClust(datain,
                                                distance = "euclidean",
                                                min.nc = 2, max.nc = 10,
                                                method = "centroid", index ="all")$Best.nc[1,])),
                                        "last")
  } else {
