  ...
  function(data,
    ...
    arimamodel <-
      forecast::auto.arima(
        ...
        max.q = maxq,
        max.d = maxd,
        allowmean = tendtomean
      )

    #### Check for ARCH ####
    Boxp <-
      stats::Box.test((arimamodel$residuals) ^ 2, lag = 12, type = "Ljung-Box")$p.value
    ...
    if (Boxp < .05) {
      last_residuals <-
        arimamodel$residuals[length(arimamodel$residuals) - ((garchmodel$order[2] - 1):0)] #last best_arch residuals
      deviations <- c()
      std_dev <-
        sqrt(
          sum(garchmodel$coef[(garchmodel$order[2] + 1):2] * (last_residuals) ^ 2) +
            garchmodel$coef[1] + garchmodel$coef[garchmodel$order[2] + 2] * arimamodel$sigma2
        ) #uses arch to predict one-step-ahead variance
      deviations[1] <- std_dev
      last_residuals[1 + garchmodel$order[2]] <- 0
      for (k in 2:forecastLength) {
        std_dev <-
          sqrt(
            sum(garchmodel$coef[(garchmodel$order[2] + 1):2] * (last_residuals[k:length(last_residuals)]) ^
                  2) +
              garchmodel$coef[1] + garchmodel$coef[garchmodel$order[2] + 2] * (deviations[k -
                                                                                            1] ^ 2)
          ) #uses arch to predict one-step-ahead variance
        deviations[k] <- std_dev
        last_residuals[k + garchmodel$order[2]] <-
          0 #future error goes here
        k <- k + 1
      }#forecast errors using ARCH. No way to get future errors?
      arima_forecasts <-
        forecast::forecast(arimamodel, forecastLength, level = 0)
      forecast_means <-
        arima_forecasts$mean
      confLevel <-
        (forecastConfidence / 100) + (1 - (forecastConfidence / 100)) / 2
      forecast_high <-
        arima_forecasts$mean + stats::qnorm(confLevel) * sqrt(cumsum(deviations ^ 2))
      forecast_low <-
        arima_forecasts$mean + stats::qnorm(1 - confLevel) * sqrt(cumsum(deviations ^ 2))
      high80 <-
        arima_forecasts$mean + stats::qnorm(.9) * sqrt(cumsum(deviations ^ 2))
      low80 <-
        arima_forecasts$mean + stats::qnorm(.1)  * sqrt(cumsum(deviations ^ 2))
      high95 <-
        arima_forecasts$mean + stats::qnorm(.975) * sqrt(cumsum(deviations ^ 2))
      low95 <-
        arima_forecasts$mean + stats::qnorm(.025) * sqrt(cumsum(deviations ^ 2))
      predictions <-
        data.frame((length(data) + 1):(length(data) + forecastLength),
                   rep(NA, forecastLength),
                   as.numeric(forecast_means),
                   as.numeric(forecast_high),
                   as.numeric(forecast_low),
                   as.numeric(high80),
                   as.numeric(low80),
                   as.numeric(high95),
                   as.numeric(low95)
      ...
      arima_forecasts <-
        forecast::forecast(arimamodel,
                           forecastLength,
                           level = c(forecastConfidence, 80, 95))
      forecast_means <-
        arima_forecasts$mean
      forecast_high <-
        arima_forecasts$upper[, 1]
      forecast_low <- arima_forecasts$lower[, 1]

      predictions <-
        data.frame((length(data) + 1):(length(data) + forecastLength),
                   rep(NA, forecastLength),
                   as.numeric(arima_forecasts$mean),
                   as.numeric(arima_forecasts$upper[, 1]),
                   as.numeric(arima_forecasts$lower[, 1]),
                   as.numeric(arima_forecasts$upper[, 2]),
                   as.numeric(arima_forecasts$lower[, 2]),
                   as.numeric(arima_forecasts$upper[, 3]),
                   as.numeric(arima_forecasts$lower[, 3])
        )
      colnames(predictions) <-
        c(
          'index',
    ...
    data.df <-
      data.frame(
        1:length(data),
        data,
        as.numeric(arima_forecasts$fitted),
        c(rep(NA, (length(data) - 1)),
          utils::tail(arima_forecasts$fitted, n = 1)),
        c(rep(NA, (length(data) - 1)),
          utils::tail(arima_forecasts$fitted, n = 1)),
    ...
    plot <-
      ggplot2::ggplot(output, ggplot2::aes_(x = ~index)) +
