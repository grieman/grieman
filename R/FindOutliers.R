...
FindOutliers <- function(x, IQRs = 3, plot = FALSE) {
  ...
  if (stats::frequency(x) > 1)
    resid <-
      stats::stl(x, s.window = "periodic", robust = TRUE)$time.series[, 3]
  else
  {
    tt <- 1:length(x)
    resid <-
      stats::residuals(stats::loess(x ~ tt, na.action = stats::na.exclude))
  }
  resid.q <- stats::quantile(resid, prob = c(0.25, 0.75), na.rm = TRUE)
  iqr <- diff(resid.q)
  limits <- resid.q + IQRs * iqr * c(-1, 1)
  score <-
    abs(pmin((resid - limits[1]) / iqr, 0) + pmax((resid - limits[2]) / iqr, 0)) #0 if not an outlier, otherwise measures how much of an outlier
  if (plot) {
    plot(x)
